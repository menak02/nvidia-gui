...
from nvidia_gui.adapters.color_xrandr import XrandrColorAdapter  # noqa: E402
from nvidia_gui.adapters.color_wayland import WaylandColorAdapter  # noqa: E402
...
def test_profile_roundtrip_new_fields() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        p.dlss_fg_preset = "b"
        p.dlss_fg_mode = "dynamic"
        p.dlss_fg_multiplier = 3
        ...
        assert loaded.dlss_fg_preset == "b"
        assert loaded.dlss_fg_mode == "dynamic"
        assert loaded.dlss_fg_multiplier == 3
        # the nested extra_env sub-table survived the TOML round-trip
        assert loaded.extra_env == {"VKD3D_CONFIG": "descriptor_indexing,dxr",
                                    "WINEESYNC": "1"}
        
        # Verify environment variables generated by services.build_game_env
        from nvidia_gui.domain.services import build_game_env
        env = build_game_env(loaded)
        assert env["DXVK_NVAPI_DRS_NGX_DLSS_FG_OVERRIDE"] == "on"
        assert env["DXVK_NVAPI_DRS_NGX_DLSS_FG_OVERRIDE_RENDER_PRESET_SELECTION"] == "b"
        assert env["DXVK_NVAPI_DRS_NGX_DLSSG_MODE"] == "dynamic"
        assert env["DXVK_NVAPI_DRS_NGX_DLSSG_MULTI_FRAME_COUNT"] == "3"
...
USER_OPTS = "PROTON_ENABLE_NVAPI=1 VKD3D_CONFIG=descriptor_indexing,dxr11 MY_VAR=1 %command% --launcher-skip"
...
def test_launch_option_inject_is_idempotent_preserves_user_and_restores() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Setup localconfig with our user options containing both curated and custom envs
        mock_vdf = _LOCALCONFIG.replace(
            '"LaunchOptions"        "PROTON_ENABLE_NVAPI=1 VKD3D_CONFIG=descriptor_indexing,dxr11 %command%"',
            f'"LaunchOptions"        "{USER_OPTS}"'
        )
        lc.write_text(mock_vdf, encoding="utf-8")

        slo = SteamLaunchOptions(steam_root=tmp)
        env_a = str(tmp / "env" / "1091500a.env")
        env_b = str(tmp / "env" / "1091500b.env")

        # 1) apply — must prepend our source, strip curated, keep custom options/args
        ...
        assert lo.endswith("--launcher-skip")
        assert "MY_VAR=1" in lo
        assert "PROTON_ENABLE_NVAPI" not in lo.split(";")[1], "curated nvapi must be stripped from user part"
        assert "VKD3D_CONFIG" not in lo.split(";")[1], "curated vkd3d must be stripped from user part"
        ...
        # 3) remove — restores the preserved user options, no source trace
        assert slo.remove_wrapper("1091500")
        lo = _locate(slo, "1091500")
        assert lo == "MY_VAR=1 %command% --launcher-skip", "remove must restore non-curated user options"
...
def test_color_xrandr_adapter_set_color() -> None:
    settings = _FakeSettings()
    with mock.patch("shutil.which", return_value="/usr/bin/xrandr"), \
         mock.patch("subprocess.run", return_value=_fake_run("", rc=0)) as run_mock:
        c = XrandrColorAdapter(settings=settings)
        assert c.is_available() is True
        
        ok = c.set_color("DP-1", 1.2, 0.8)
        assert ok is True
        run_mock.assert_called_once_with(
            ["/usr/bin/xrandr", "--output", "DP-1", "--brightness", "1.2", "--gamma", "0.8:0.8:0.8"],
            capture_output=True, text=True, timeout=5, check=False
        )
        assert settings.get("color.DP-1.brightness") == 1.2
        assert settings.get("color.DP-1.contrast") == 0.8


def test_color_wayland_adapter_hyprland_set_color() -> None:
    settings = _FakeSettings()
    with mock.patch("shutil.which", side_effect=lambda x: "/usr/bin/wlr-randr" if x == "wlr-randr" else ("/usr/bin/hyprctl" if x == "hyprctl" else None)), \
         mock.patch("os.environ", {"HYPRLAND_INSTANCE_SIGNATURE": "test_sig"}), \
         mock.patch("builtins.open", mock.mock_open()) as open_mock, \
         mock.patch("subprocess.run", return_value=_fake_run("", rc=0)) as run_mock:
        
        c = WaylandColorAdapter(settings=settings)
        assert c.is_available() is True
        
        ok = c.set_color("DP-1", 1.2, 0.8)
        assert ok is True
        
        # Should verify hyprctl eval command
        run_mock.assert_called_once_with(
            ["hyprctl", "eval", "hl.config({ decoration = { screen_shader = '/tmp/nvgui_color.frag' } })"],
